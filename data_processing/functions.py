...
# Function for Initializing Database Schema
def initialize_schema_raw(connection, cursor):
    # SQL query for creating database schemas and tables for each schema. Here, the schema 'landing' stores the raw data, whilst the
    # 'staging' schema stores the processed, normalized data from the 'landing' schema. Lastly, the 'aggregate' schema stores the aggregated
    # data from the 'staging' schema
    query = '''
        CREATE SCHEMA IF NOT EXISTS landing;
        CREATE SCHEMA IF NOT EXISTS staging;
        CREATE SCHEMA IF NOT EXISTS aggregate;

        CREATE TABLE IF NOT EXISTS landing.raw_rto_demand (
            id_demand VARCHAR PRIMARY KEY,
            time_stamp VARCHAR,
            rto_name VARCHAR,
            rto_full_name VARCHAR,
            demand_mwh VARCHAR,
            inserted_on TIMESTAMP DEFAULT clock_timestamp()
        );

        CREATE TABLE IF NOT EXISTS landing.raw_fuel_electricity_generation (
            id_elec_generated VARCHAR PRIMARY KEY,
            time_stamp VARCHAR,
            rto_name VARCHAR,
            rto_full_name VARCHAR,
            fuel_name VARCHAR,
            fuel_full_name VARCHAR,
            mwh_generated VARCHAR,
            inserted_on TIMESTAMP DEFAULT clock_timestamp()
        );


        CREATE TABLE IF NOT EXISTS staging.rto_details (
            rto_id VARCHAR(50) PRIMARY KEY,
            rto_full_name VARCHAR,
            updated_at TIMESTAMP DEFAULT clock_timestamp()
        );

        CREATE TABLE IF NOT EXISTS staging.electricity_demands (
            demand_id VARCHAR(50) PRIMARY KEY,
            time_stamp TIMESTAMP,
            rto_id VARCHAR(50),
            demand_mwh NUMERIC,
            updated_at TIMESTAMP DEFAULT clock_timestamp(),
            CONSTRAINT fk_rto_fuel FOREIGN KEY (rto_id) REFERENCES staging.rto_details(rto_id)
            ON DELETE CASCADE
        );

        CREATE TABLE IF NOT EXISTS staging.fuel_type_generation (
            fuel_generation_id VARCHAR(50) PRIMARY KEY,
            time_stamp TIMESTAMP,
            rto_id VARCHAR(50),
            mwh_ng NUMERIC,
            mwh_nuc NUMERIC,
        ...
        DROP MATERIALIZED VIEW IF EXISTS aggregate.view_demand_electricty_generation_mlready;

        CREATE MATERIALIZED VIEW aggregate.view_demand_electricty_generation_mlready AS
        SELECT
            CONCAT('DF', UPPER(d.rto_id), TO_CHAR(d.time_stamp, 'YYYYMMDD-HH24'), 'AGG') AS id_demand_elec_generated,
            d.time_stamp, d.rto_id, r.rto_full_name, d.demand_mwh,
            f.mwh_ng, f.mwh_nuc, f.mwh_wnd, f.mwh_sun, f.mwh_col, f.mwh_wat, f.mwh_oil, f.mwh_oth
        FROM staging.electricity_demands d
        LEFT JOIN staging.rto_details r ON r.rto_id = d.rto_id
        LEFT JOIN staging.fuel_type_generation f ON f.time_stamp = d.time_stamp AND f.rto_id = d.rto_id
        ORDER BY d.rto_id ASC, d.time_stamp asc;
    '''
...
# Function for Inserting Raw Data in Bulk to the Database
def extract_insert_data(start:str, end:str, api_key:str, institutions:list, username_postgres: str, pw_postgres: str, db_name: str='data_warehouse_electricity'):
    connection, cursor = initialize_database(user=username_postgres, pw=pw_postgres, db_name=db_name) # initializing & connecting to the database
    initialize_schema_raw(connection, cursor) # initializing & creating database schema
    ...
    table_dict = {
        'raw_rto_demand': extract_demands_institutions(start=start, end=end, api_key=api_key, institutions=institutions, folder_name='raw', max_workers=6), 
        'raw_fuel_electricity_generation': extract_values_fuel_type(start=start, end=end, api_key=api_key, institutions=institutions, folder_name='raw', max_workers=6)
    }

    # Looping through the dictionary, and doing bulk inserts from the saved .csv files of the dataframes into the database
    for tbl, (data, path) in table_dict.items():
        columns = sql.SQL(', ').join([sql.Identifier(col) for col in data.columns]) # creating columns for the SQL query

        cursor.execute(sql.SQL('TRUNCATE TABLE landing.{tbl_name}').format(tbl_name=sql.Identifier(tbl))) # truncating the table, since this function is supposed to insert data into an empty database
        with open(path, 'r', encoding='utf-8') as f:
            # Copying & inserting data from the .csv file into the database
            query_copy = sql.SQL("COPY landing.{tbl_name} ({cols}) FROM STDIN WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');").format(
                tbl_name=sql.Identifier(tbl), cols=columns
            )
            cursor.copy_expert(sql=query_copy, file=f)
            connection.commit() # commit changes
            print(f'\nSaved {tbl} to database')

    insert_to_staging(connection, cursor, insert_raw_data=True) # inserts the data into the staging schema
    print('Inserted data to staging')
